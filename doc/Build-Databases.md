...
Compared with the snappy compression used up to 0.3, the records take a third of the space and read back just as fast. The LMDB file of a Crossref database loaded this way is about 40% the size of the same database loaded with snappy (measured with `./gradlew crossref` on 28,495 records: 40 MB against 98 MB); the file shrinks less than the records do because the per-page overhead of LMDB is the same in both cases. Zstandard without a dictionary is not worth it for records this size: it only saves a quarter and decompresses twice as slowly, and LZ4 saves nothing over snappy. Higher zstd levels shave off a few more percent while making the load several times slower, which is why the default level is 3; a full Crossref load is bound by parsing, not by compression, at that level.
