...
public class LoadCrossrefCommand extends ConfiguredCommand<LookupConfiguration> {
    ...
    public static final String CROSSREF_SOURCE_TYPE = "crossref.sourceType";
    ...
    public void configure(Subparser subparser) {
        ...
        String validSources = Stream.of(SourceType.values())
                .map(SourceType::name)
                .collect(Collectors.joining(", "));

        subparser.addArgument("--type")
                .dest(CROSSREF_SOURCE_TYPE)
                .type(String.class)
                .required(true)
                .help("The crossref dump source type ("+validSources+").");
    }
    ...
    protected void run(Bootstrap bootstrap, Namespace namespace, LookupConfiguration configuration) throws Exception {
        ...
        final String crossrefDumpSourceType = namespace.get(CROSSREF_SOURCE_TYPE);
        ...
        switch( SourceType.valueOf( crossrefDumpSourceType ) ) {
            case TORRENT:
            ...
            case METADATAPLUS:
                if (Files.isRegularFile(crossrefFilePath) && Files.isReadable(crossrefFilePath)
                        && (StringUtils.endsWithIgnoreCase(crossrefFilePath.getFileName().toString(), ".tar.gz"))) {
                    TarArchiveInputStream tarInput = new TarArchiveInputStream(new GzipCompressorInputStream(Files.newInputStream(crossrefFilePath)));
